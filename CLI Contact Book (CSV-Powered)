...
def update_contact():
    ...
    with open(FILENAME, "a", encoding="utf-8") as f:
      update = csv.writer(f)
      update.writerow([name, phone, email])
      print("Contact updated.")
      
def delete_contact():
    name_to_delete = input("Enter the name to be deleted: ").lower().strip()
    updated_rows = []
    ...
    for row in rows[1:]:
        if row and row[0].lower() != name_to_delete:
            updated_rows.append(row)
            
    #write the data in deleted one
    with open(FILENAME, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            print("Invalid choice for number")
